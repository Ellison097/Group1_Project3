...
def validate_response(response: requests.Response, doi: str) -> Dict[str, Any]:
    """
    Validate and parse API response
    
    Args:
        response: API response object
        doi: DOI being searched
    ...
    try:
        ...
        if not isinstance(data, dict):
            raise CitationFetchError(f"Invalid response format for DOI '{doi}'")
        return data
    except requests.exceptions.JSONDecodeError:
        raise CitationFetchError(f"Invalid JSON response for DOI '{doi}'")
    except requests.exceptions.HTTPError as e:
        if e.response.status_code == 429:
            raise RateLimitError("Rate limit exceeded")
        raise CitationFetchError(f"HTTP error {e.response.status_code}")

def process_search_results(data: Dict[str, Any], doi: str) -> Optional[int]:
    """
    Process search results and extract citation count
    
    Args:
        data: API response data
        doi: Original search DOI
    ...
    if not isinstance(first_result, dict):
        raise CitationFetchError(f"Invalid result format for DOI '{doi}'")
        
    citation_count = first_result.get("cited_by_count")
    if citation_count is not None and not isinstance(citation_count, (int, float)):
        raise CitationFetchError(f"Invalid citation count format for DOI '{doi}'")
        
    return citation_count

def fetch_citations(df: pd.DataFrame) -> List[float]:
    """
    Fetches citation counts from OpenAlex using DOI search.
    ...
    citation_counts = []
    total_rows = len(df)
    logging.info(f"Starting citation fetch for {total_rows} records using DOI search.")
    ...
    for index, row in tqdm(df.iterrows(), total=total_rows, desc="Fetching Citations"):
        ...
        # Validate DOI
        if not (pd.notna(doi) and isinstance(doi, str) and doi.strip()):
            logging.debug(f"Row {index}: Skipping due to missing or invalid DOI.")
        ...
        try:
            ...
            search_query = urllib.parse.quote_plus(doi.strip())
            api_url = f"https://api.openalex.org/works?filter=doi:{search_query}"
            ...
            data = validate_response(response, doi)
            citation_count = process_search_results(data, doi) or np.nan
            
            logging.debug(
                f"Row {index}: DOI '{doi}' - Found count {citation_count}"
            )
            ...
            logging.error(f"Row {index}: DOI '{doi}' - {str(e)}")
        except requests.exceptions.Timeout:
            logging.error(f"Row {index}: DOI '{doi}' - Request timed out.")
        except requests.exceptions.RequestException as e:
            logging.error(f"Row {index}: DOI '{doi}' - Request Error: {e}")
        except Exception as e:
            logging.error(f"Row {index}: DOI '{doi}' - Unexpected error: {e}")
